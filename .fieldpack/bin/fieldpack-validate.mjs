...
// ../.fieldpack/schemas/v1.0/service.yaml.schema.json
var service_yaml_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://dev.fieldpack.co/schemas/v1.0/service.yaml.schema.json",
  title: "Fieldpack service.yaml v1.0",
  type: "object",
  additionalProperties: false,
  required: ["schema_version", "name", "description", "kind", "status", "visibility", "owner", "language", "repo"],
  properties: {
    schema_version: { type: "string", const: "1.0" },
    name: { type: "string", pattern: "^[a-z][a-z0-9-]*[a-z0-9]$", minLength: 2, maxLength: 64 },
    description: { type: "string", minLength: 10, maxLength: 280 },
    kind: { enum: ["service", "tool", "hardware", "docs"] },
    status: { enum: ["active", "wip", "archived", "deprecated"] },
    ...
    deploy: {
      ...
      additionalProperties: false,
      properties: {
        target: { enum: ["netlify", "vercel", "cloudflare", "none"] },
        url: { type: "string", format: "uri" }
      }
    },
    repo: { type: "string", format: "uri", pattern: "^https://github.com/" },
    docs: {
      type: "object",
      additionalProperties: false,
      properties: {
...
// ../.fieldpack/schemas/v1.1/service.yaml.schema.json
var service_yaml_schema_default2 = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://dev.fieldpack.co/schemas/v1.1/service.yaml.schema.json",
  title: "Fieldpack service.yaml v1.1",
  type: "object",
  additionalProperties: false,
  required: ["schema_version", "name", "description", "kind", "status", "visibility", "owner", "language", "repo"],
  properties: {
...
// src/lib/types.ts
var Kind = external_exports.enum(["service", "tool", "hardware", "docs"]);
var Visibility = external_exports.enum(["public", "internal"]);
var Status = external_exports.enum(["active", "wip", "archived", "deprecated"]);
var DeployV10 = external_exports.object({
  target: external_exports.enum(["netlify", "vercel", "cloudflare", "none"]).optional(),
  url: external_exports.string().url().optional()
}).strict();
var DeployV11 = external_exports.object({
  target: external_exports.enum(["netlify", "vercel", "cloudflare", "none"]).optional(),
  url: external_exports.string().url().optional(),
  netlify: external_exports.object({
    site_id: external_exports.string().min(1).optional(),
    site_name: external_exports.string().min(1).optional()
  }).strict().optional()
}).strict();
var BaseFields = {
  name: external_exports.string().regex(/^[a-z][a-z0-9-]*[a-z0-9]$/).min(2).max(64),
  ...
  repo: external_exports.string().url().startsWith("https://github.com/"),
  docs: external_exports.object({
    homepage: external_exports.string().optional(),
    architecture: external_exports.string().optional(),
    llms: external_exports.string().optional()
  }).optional(),
  depends_on: external_exports.array(external_exports.string().regex(/^[a-z][a-z0-9-]*[a-z0-9]$/)).optional(),
  tags: external_exports.array(external_exports.string()).optional()
};
var ServiceYamlV10 = external_exports.object({
  schema_version: external_exports.literal("1.0"),
  ...BaseFields,
  deploy: DeployV10.optional()
}).strict();
var ServiceYamlV11 = external_exports.object({
  schema_version: external_exports.literal("1.1"),
  ...BaseFields,
  deploy: DeployV11.optional(),
  secrets: external_exports.array(external_exports.string().regex(/^[A-Z][A-Z0-9_]*$/)).optional()
}).strict();
var ServiceYaml = external_exports.union([ServiceYamlV10, ServiceYamlV11]);

// src/lib/schema.ts
var ServiceYamlError = class extends Error {
  constructor(message, cause) {
    super(message);
    this.cause = cause;
    this.name = "ServiceYamlError";
  }
  cause;
};
...
var validators = {
  "1.0": ajv.compile(service_yaml_schema_default),
  "1.1": ajv.compile(service_yaml_schema_default2)
};
function pickVersion(parsed) {
...
function validateServiceYaml(source) {
  let parsed;
  try {
    parsed = jsYaml.load(source, { json: true });
  } catch (err) {
    throw new ServiceYamlError("Malformed YAML", err);
  }
  const version = pickVersion(parsed);
  ...
  const zodSchema = version === "1.1" ? ServiceYamlV11 : ServiceYamlV10;
...
// src/lib/status-md.ts
function ageInDays(mtimeMs, nowMs = Date.now()) {
  return Math.floor((nowMs - mtimeMs) / (1e3 * 60 * 60 * 24));
}

// src/lib/compliance.ts
var SUPPORTED_FIELDPACK_VERSIONS = /* @__PURE__ */ new Set(["1.0", "1.1"]);
var REQUIRED_FILES = [
  "service.yaml",
  "README.md",
  "STATUS.md",
  ...
  "docs/llms.md",
  ".fieldpack/version",
  ".fieldpack/CHARTER.md",
  ".fieldpack/AGENT_RULES.md",
  ".fieldpack/scripts/validate.sh"
];
var VENDORED_VALIDATOR_PATH = ".fieldpack/bin/fieldpack-validate.mjs";
function requiredFilesForFieldpackVersion(version) {
  return version === "1.1" ? [...REQUIRED_FILES, VENDORED_VALIDATOR_PATH] : [...REQUIRED_FILES];
}
function schemaMirrorPath(schemaVersion) {
  return `.fieldpack/schemas/v${schemaVersion}/service.yaml.schema.json`;
}
var REQUIRED_DIRS = ["docs/prd", "docs/decisions", ".fieldpack/templates"];
var STALE_DAYS = 30;
var GUIDE_MARKDOWN_DIRS = /* @__PURE__ */ new Set(["projects/guides", "guides"]);
...
function isMarkdownGuidePath(relativePath) {
...
async function checkCompliance(repoRoot, ctx) {
  const failures = [];
  const versionPath = path.join(repoRoot, ".fieldpack/version");
  const declaredFieldpackVersion = await isFile(versionPath) ? (await fs.readFile(versionPath, "utf8")).trim() : null;
  for (const file of requiredFilesForFieldpackVersion(declaredFieldpackVersion ?? "1.0")) {
    if (!await isFile(path.join(repoRoot, file))) {
      failures.push({ code: "missing-file", path: file });
    }
  }
  if (declaredFieldpackVersion !== null && !ctx.supportedFieldpackVersions.has(declaredFieldpackVersion)) {
    failures.push({
      code: "unsupported-fieldpack-version",
      path: ".fieldpack/version",
      detail: `declared "${declaredFieldpackVersion}"; supported: ${[...ctx.supportedFieldpackVersions].join(", ")}`
    });
  }
  for (const dir of REQUIRED_DIRS) {
    if (!await isDirectory(path.join(repoRoot, dir))) {
      failures.push({ code: "missing-dir", path: dir });
    }
  }
  let parsedService = null;
  const servicePath = path.join(repoRoot, "service.yaml");
  if (await isFile(servicePath)) {
    const source = await fs.readFile(servicePath, "utf8");
    try {
      const result = validateServiceYaml(source);
      if (result.ok) {
        parsedService = result.value;
      } else {
  ...
  if (parsedService) {
    const required = schemaMirrorPath(parsedService.schema_version);
    if (!await isFile(path.join(repoRoot, required))) {
      failures.push({ code: "missing-file", path: required });
    }
  }
  if (parsedService && ctx.knownServices) {
    for (const dependency of parsedService.depends_on ?? []) {
      if (!ctx.knownServices.has(dependency)) {
        failures.push({
          code: "unknown-dependency",
          path: "service.yaml",
          detail: `depends_on: ${dependency}`
        });
      }
    }
  }
  const statusPath = path.join(repoRoot, "STATUS.md");
  if (await isFile(statusPath)) {
    const status = await fs.stat(statusPath);
    const age = ageInDays(status.mtimeMs, ctx.now);
    if (age > STALE_DAYS) {
      failures.push({ code: "stale-status", path: "STATUS.md", detail: `${age} days old` });
    }
  }
  const claudePath = path.join(repoRoot, "CLAUDE.md");
  if (await isFile(claudePath)) {
    const text = await fs.readFile(claudePath, "utf8");
    if (!/AGENTS\.md|AGENT_RULES\.md/.test(text)) {
      failures.push({
        code: "missing-charter-ref",
        path: "CLAUDE.md",
        detail: "no reference to AGENTS.md or .fieldpack/AGENT_RULES.md"
      });
    }
  }
  const agentsPath = path.join(repoRoot, "AGENTS.md");
  if (await isFile(agentsPath)) {
    const text = await fs.readFile(agentsPath, "utf8");
    if (!/CHARTER\.md/.test(text)) {
      failures.push({
        code: "missing-charter-ref",
        path: "AGENTS.md",
        detail: "no reference to .fieldpack/CHARTER.md"
      });
    }
  }
...
// src/commands/validate.ts
async function validateCommand(repoRoot = process.cwd()) {
  const result = await checkCompliance(repoRoot, {
    supportedFieldpackVersions: SUPPORTED_FIELDPACK_VERSIONS
  });
  if (result.compliant) {
    console.log(import_picocolors.default.green("\u2713 compliant"));
    console.log(import_picocolors.default.dim("  (depends_on closure is enforced by the Shopkeep build, not by `fieldpack validate`)"));
    return 0;
  }
  console.log(import_picocolors.default.red(`\u2717 ${result.failures.length} failure(s):`));
  for (const failure of result.failures) {
    const where = failure.path ? ` (${failure.path})` : "";
    const detail = failure.detail ? `: ${failure.detail}` : "";
    console.log(`  ${import_picocolors.default.red("\xD7")} ${failure.code}${where}${detail}`);
  }
  return 1;
}

// src/bin/fieldpack-validate.ts
process.exitCode = await validateCommand(process.cwd());
