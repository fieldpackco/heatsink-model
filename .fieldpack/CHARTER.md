...
`docs/reviews/` handoffs coordinate agents **inside** one repo. They are not an inbox another repo watches: nothing polls them across a boundary, so a request left in your own reviews reaches the other side only if a human carries it. When work touches a boundary, the rules below apply.
...
- **Reaching in includes the filesystem.** On a shared machine every repo's checkout is physically writable; that does not make it yours. Never edit, commit, or tag in a working tree that belongs to another repo or another live session — even a correct change lands as a race: it gets swept into someone else's commit, attributed to nobody, and if it was an allocation (a family value, a presence bit, a port, an ID), two sessions can hand out the same number with two meanings. The owning repo's session makes the edit; your request travels by this section's reporting path, never by the other repo's working tree.
- **Reading across a boundary is fine; depending on it is not.** Read another repo's code, docs, contracts and review threads freely — understanding what you are integrating with is the job. What must not happen is treating those files as a channel: leaving a request in a place the other side has no reason to look, or acting on a handoff addressed to someone else. A Linear issue may cite a document in your repo for detail that does not fit an issue; the routing is the issue, and the document is evidence.
